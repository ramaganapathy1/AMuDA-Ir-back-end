...
"""for i in range(0, len(edgeConWT)):
    edgeConWT[i] = (1 - float((edgeConWT[i] - minC) / (maxC - minC)))
print edgeConWT
...
for i in range(0, len(edgeConWT)):
    ...
    if (edgeConWT[i] > (1.0 * edgeElaWT[i])):
        if doc1.index(edgeStart[i]) != doc1.index(edgeEnd[i]):
            t = "v" + str(doc1.index(edgeStart[i])) + ",v" + str(doc1.index(edgeEnd[i])) + ",false," + str(
                edgeConWT[i]) + ",#FF0000,true\n"  # red
            if(t not in listt):
                fdFinal.write(t)
                listt.append(t)
    else:
        t = "v" + str(doc1.index(edgeStart[i])) + ",v" + str(doc1.index(edgeEnd[i])) + ",false," + str(
            edgeElaWT[i]) + ",#228B22,true\n"  # green
        if (t not in listt):
            fdFinal.write(t)            listt.append(t)
print "Total edges : ",len(listt)
